  ...
  return (
    <div className="dash-wrapper">
      ...
      <main className="content">
        ...
        {tab === 'roster'       && <Roster rosters={r} userMap={userMap} players={players} userId={userId} isLoading={playersLoading} salaries={salaries} setSalary={setSalary} cap={cap} leagueId={leagueId} />}
        {tab === 'playoffs'     && <Bracket winners={winners ?? []} losers={losers ?? []} rosters={r} userMap={userMap} isLoading={winnersLoading || losersLoading} />}
        {tab === 'draft'        && <DraftRecap picks={draftPicks ?? []} rosters={r} userMap={userMap} isLoading={draftLoading} players={players} />}
        ...
        {tab === 'draft-board'  && <DraftBoard players={players} leagueId={leagueId!} teamCount={league?.settings?.num_teams ?? 12} isLoading={playersLoading} salaries={salaries} cap={cap} rosters={r} userMap={userMap} userId={userId} />}
        </div>
        </div>
      </main>
      </div>
