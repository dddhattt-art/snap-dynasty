...
function pf(roster: SleeperRoster): number {
...
function getFinalists(bracket: SleeperBracketMatch[]): {
  ...
  const championship = finals[0];
  const thirdGame = finals[1];
  return {
    first: championship?.w ?? null,
    second: championship?.l ?? null,
    third: thirdGame?.w ?? null,
  };
}
...
export default function LeagueHistory({ history, allTimeMatchups, userId, isLoading, allTimeLoading }: Props) {
  ...
  // Build user lookup from all seasons (latest appearance wins)
  ...
  const initCareer = (uid: string): CareerStat => ({
  ...
  for (const { league, rosters, bracket } of history) {
    ...
    for (const roster of rosters) {
      ...
      if (!ownerId) continue;
      if (!career.has(ownerId)) career.set(ownerId, initCareer(ownerId));
  ...
  const rows = [...career.values()]
    ...
    .sort((a, b) => {
      const champDiff = b.championships - a.championships;
  ...
  // ── H2H from allTimeMatchups ─────────────────────────────────────
  type H2HRecord = { wins: number; losses: number };
  const h2h = useMemo((): Map<string, Map<string, H2HRecord>> => {
    const result = new Map<string, Map<string, H2HRecord>>();
    if (!allTimeMatchups) return result;
  ...
  // ── Record book from allTimeMatchups ─────────────────────────────
  type RecordEntry = { label: string; value: string; detail: string; icon: string };
  const records = useMemo((): RecordEntry[] => {
    if (!allTimeMatchups) return [];
    const entries: RecordEntry[] = [];

    let highScore = { score: 0, owner: '', season: '', week: 0 };
    let biggestBlowout = { margin: 0, winOwner: '', loseOwner: '', season: '', week: 0 };
    const seasonPF = new Map<string, number>(); // ownerId -> max single-season PF
    ...
    for (const { league, rosters } of history) {
      ...
      // Season PF from roster settings
      for (const roster of rosters) {
        const ownerId = rosterOwner.get(roster.roster_id);
        if (!ownerId) continue;
        const seasonPts = pf(roster);
        if (seasonPts > (seasonPF.get(ownerId) ?? 0)) seasonPF.set(ownerId, seasonPts);
      }
      ...
      for (const [weekStr, weekMatchups] of Object.entries(weekMap)) {
        ...
        for (const matchup of weekMatchups) {
          if (matchup.points > highScore.score) {
            highScore = {
              score: matchup.points,
              owner: rosterOwner.get(matchup.roster_id) ?? '',
    ...
    // Best single-season PF from career stats
    const bestSeasonPF = [...rows].sort((a, b) => {
      const aMax = seasonPF.get(a.userId) ?? 0;
      const bMax = seasonPF.get(b.userId) ?? 0;
      return bMax - aMax;
    })[0];
    if (bestSeasonPF) {
      const max = seasonPF.get(bestSeasonPF.userId) ?? 0;
      if (max > 0) entries.push({
        icon: 'star',
        label: 'Most Points in a Season',
        value: max.toFixed(2),
        detail: name(bestSeasonPF.userId),
      });
    }
    ...
    // Most championships
    if (mostChamps?.championships > 1) entries.push({
      icon: 'crown',
      label: 'Most Championships',
      value: `${mostChamps.championships}×`,
      detail: name(mostChamps.userId),
    });
    ...
    // Best win rate (min 2 seasons)
    const qualifiedBestWR = rows.filter(r => r.seasons >= 2)[0];
    if (qualifiedBestWR) {
      const wp = (qualifiedBestWR.wins / (qualifiedBestWR.wins + qualifiedBestWR.losses || 1) * 100).toFixed(1);
      entries.push({
        icon: 'percentage',
        label: 'Best Win Rate (2+ seasons)',
        value: `${wp}%`,
        detail: name(qualifiedBestWR.userId),
      });
    }
    ...
    return entries;
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [allTimeMatchups, rows]);

  // Versus target default to logged-in user
    ...
    <div className="ath-wrap">
      ...
      <div className="ath-strip">
        <div className="ath-strip-stat">
          <div className="ath-strip-val">{totalSeasons}</div>
      ...
      <div className="ath-tabs">
        {(['standings', 'seasons', 'versus', 'records'] as View[]).map(v => (
          <button
            key={v}
            className={`ath-tab${view === v ? ' active' : ''}`}
            onClick={() => setView(v)}
          >
                  ...
                  <tr key={c.userId} className={isMe ? 'ath-me-row' : ''}>
                    ...
                    <td>
                      <div className="ath-trophies">
                        {c.championships > 0 && (
                          <span className="ath-trophy-pill gold">
                            <i className="ti ti-trophy" /> {c.championships}
                          </span>
                        )}
                        {c.runnerUp > 0 && (
                          <span className="ath-trophy-pill silver">
                            <i className="ti ti-medal" /> {c.runnerUp}
                          </span>
                        )}
                        {c.thirdPlace > 0 && (
                          <span className="ath-trophy-pill bronze">
                            <i className="ti ti-medal-2" /> {c.thirdPlace}
                          </span>
                        )}
          ...
          <table className="standings-table">
            ...
            <tbody>
              {seasons.map(({ league, rosters, bracket }) => {
                ...
                const isMyChamp = champUid === userId;

                return (
                  <tr key={league.season} className={isMyChamp ? 'ath-me-row' : ''}>
                    <td><span className="ath-season-badge">{league.season}</span></td>
                    <td>
                      <div className="team-cell">
                        <span className="ath-season-champ-icon">🏆</span>
                        <span className="ath-bold">
        ...
        <div className="ath-versus">
          <div className="ath-versus-picker">
            ...
            <select
              ...
              onChange={e => setVersusTarget(e.target.value)}
            >
              {rows.map(r => (
          ...
          {allTimeLoading ? (
            ...
            <div className="snap-empty">
              <i className="ti ti-refresh" style={{ fontSize: 28, opacity: 0.3 }} />
              <p>Head-to-head data loads with the Record Book. Switch to Record Book tab to trigger the load.</p>
            </div>
          ) : (
            (() => {
                  ...
                  <div className="ath-versus-summary">
                    <div className="ath-versus-av-wrap">
                      {(() => {
                        ...
                        const name = u?.display_name ?? u?.username ?? effectiveTarget;
                        return av
                          ? <img src={av} alt="" className="ath-versus-av" />
                          : <div className="ath-versus-av ath-versus-av-init" style={{ background: color + '22', color }}>{initials(name)}</div>;
                      })()}
                    </div>
                    <div className="ath-versus-name">{userName(effectiveTarget)}</div>
                    ...
                    <table className="standings-table">
                      ...
                      <tbody>
                        {opponents.map(opp => {
                          ...
                          const u = latestUserMap.get(opp.userId);
                          const av = u ? avatarUrl(u.avatar) : null;
                          const color = userColorMap.get(opp.userId) ?? '#6b7280';
                          const name = u?.display_name ?? u?.username ?? opp.userId;
                            ...
                            <tr key={opp.userId}>
                              <td>
                                <div className="team-cell">
                                  {av
                                    ? <img loading="lazy" src={av} alt="" className="avatar-xs" />
                                    : <div className="ath-av-init" style={{ background: color + '22', color }}>{initials(name)}</div>
                                  }
                                  <span>{name}</span>
                                </div>
                              </td>
                              ...
                              <td>{wp}{total > 0 ? '%' : ''}</td>
                              <td>
                                {total === 0 ? <span className="ath-muted">No matchups</span>
        ...
        <div className="ath-records">
          {allTimeLoading ? (
            ...
            <div className="snap-empty">
              ...
              <p>Record book data is loading. This may take a moment for leagues with many seasons.</p>
            </div>
          ) : records.length === 0 ? (
            ...
            <div className="ath-records-grid">
              {records.map((rec, i) => (
                <div key={i} className="ath-record-card">
                  <div className="ath-record-icon">
                    <i className={`ti ti-${rec.icon}`} />
                  </div>
